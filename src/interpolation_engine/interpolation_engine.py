# This is meant to run the new 'program' format that incudes the key 'default_state'.
"""

GENERAL ARCHITECTURE
This script expects to be passed a JSON5 file, this file is called a 'program' as it defines
...
A 'task', is an object of the structure {'cmd': 'print', 'text':'Hello!\n'}
The string values of a task can be interpolated, e.g. {'cmd': 'print', 'text':'My name is {name}.'}
...
The order of executed tasks is also affected by 'goto', 'goto_map', 'for' and 'for'.

Tasks are usually defined directly in the order, they may also be defined in program['tasks'],
where their key is a name by which they can be executed using e.g. {'cmd':'run_task', 'task_name':'mytask'}.

The values that can be interpolated into strings are called 'inserts'.

Since the program['order'] and  program['tasks'] are static, all the variable runtime information is contained in it's so called 'state'.
...
Special Interpolation keys:
    ...
    'ARG1': 'The first argument passed into the program, only defined if one was passed. insert_start and insert_stop will be escaped.
    'ARG2': 'The second argument passed into the program, only defined if one was passed. insert_start and insert_stop will be escaped.
    'ARG{n}': 'The n-th argument passed into the program, only defined if one was passed. insert_start and insert_stop will be escaped.


Special Labels:
    'CONTINUE': If a goto_map or goto point at this label, it will be a no-op and the next order item
        will be executed.
...
# Delete Except
delete_except is useful to clean up the inserts. use 'wildcards' to specify a list of names which,
as the name suggests, support wildcards. If you no longer need to store any information except the users
preferences, call {cmd:'delete_except', wildcards:['user/*']}. This assumes that you stored the user
information as e.g. 'user/name'.
use {cmd:'delete', '
...
goto_map:
    ...
    Inputs:
        ...
        - target_maps (list[dict]) : a list of dicts, each dict consists of only one key, representing
            the string value_text has to match. The value is the name of the label it will go to if
            the key matches. The keys may use wildcards, '*', to match patterns or the special key,
            'NULL', to check if an interpolation in value_text could not be resolved. The list order is
            relevant as the first match will be used.
    Output:
        None
    Example:
    ```
...
# We need prompt_toolkit to listen for keyboard shortcuts such as ctrl-o and ctrl-s when asking the user for `input()`.
